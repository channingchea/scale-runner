...
Independent of Guitar Mode; both are live bugs on piano today.
- [ ] `quiz_settings.dart`: add `runShowDots()` / `setRunShowDots(bool)`
      (`run_show_dots`, default true), same pattern as `invShowDots()`.
- [ ] `scale_run_settings_sheet.dart`: dots switch, same copy as the Inversion
      sheet. `scale_run_screen.dart:589` becomes
      `isTargetHint: _showDots ? c.isTargetHint : (_) => false`.
- [ ] Samples: generate `note_40.wav` to `note_47.wav` and `note_78.wav` to
      `note_88.wav` matching the existing files (22050 Hz mono, 1.1 s, same
      envelope and level; derive the recipe from `note_60.wav` since the
      script was never committed). **Commit the generator to `tool/` this
      time.** Set `NotePlayer.lowMidi = 40`, `highMidi = 88`, fix its doc
      comment. Nothing changes in `pubspec.yaml` (the directory is declared).
- [ ] Tests: `voicing_settings_test.dart`-style round trip for the new pref;
      a test that every MIDI note any cycle can produce (`InversionCycle` for
      all chords x 12 keys, `VoicingCycle` over the 48-84 keyboard) is inside
      `NotePlayer`'s range.
- [ ] `flutter analyze` + `flutter test`, bump build, ship to beta.
